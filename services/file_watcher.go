...
type AIAnalysisResponse struct {
	...
	ThoughtsTH      string              `json:"thoughts_th"`
...
func recordAIAction(userID string, fileID uint, action, description, folder, filename, originalPath string, isMove bool, confidence int, insights string) {
	status := "success"
...
func processNewFile(sourcePath, fileName string, userID string, userAIConfig models.UserAIConfig) {
	...
	metadata.SummaryTH = aiResp.SummaryTH
	...
	saveFileTags(metadata.ID, aiResp.Tags)
	...
	if userAIConfig.DestinationPath != "" && userAIConfig.AutoSelectFolder {
		...
		insightsPayload := map[string]interface{}{
			"thoughts":    aiResp.Thoughts,
			"thoughts_th": aiResp.ThoughtsTH,
			"raw_prompt":  aiResp.RawPrompt,
			"entities":    aiResp.Entities,
			"tags":        aiResp.Tags,
			"summary":     aiResp.Summary,
			"summary_th":  aiResp.SummaryTH,
		}
		insightsJSON, _ := json.Marshal(insightsPayload)

		recordAIAction(userID, metadata.ID, "auto_organize",
			fmt.Sprintf("Automatically organized '%s' into '%s'", fileName, selectedFolder),
			selectedFolder, destFileName, sourcePath, true, aiResp.ConfidenceScore, string(insightsJSON))

		// Feed auto-move result into personalization learning pipeline
		// This updates keyword weights, centroid vectors, and accept counts
		go RecordDecisionEvent(DecisionEventInput{
			UserID:          userID,
			FileID:          metadata.ID,
			Source:          "auto_watcher",
			Outcome:         "auto_moved",
		...
		recordAIAction(userID, metadata.ID, "suggestion",
			fmt.Sprintf("Suggested moving '%s' to '%s' (Confidence: %d%%)", fileName, selectedFolder, aiResp.ConfidenceScore),
			selectedFolder, fileName, sourcePath, false, aiResp.ConfidenceScore, string(insightsJSON))
	}
}
...
func ScanOrigin(userID string, customPath string, userPrompt string) (int, error) {
	...
	for _, f := range filesToProcess {
		...
		if err == nil {
			...
			saveFileTags(meta.ID, aiResp.Tags)
		}
...
func getFolderProfiles(userID string) map[string]string {
	...
	for _, p := range dbProfiles {
		// Build rich description including accept rate for better AI context
		total := p.AcceptCount + p.RejectCount
		aDesc := p.Description
		if total > 0 {
			acceptRate := int(float64(p.AcceptCount) / float64(total) * 100)
			if aDesc != "" {
				aDesc = fmt.Sprintf("%s [confidence: %d%%]", aDesc, acceptRate)
			} else {
				aDesc = fmt.Sprintf("[confidence: %d%%]", acceptRate)
			}
		}
		profiles[p.FolderName] = aDesc
	}
	return profiles
}
...
func saveFileTags(fileID uint, tagNames []string) {
	if len(tagNames) == 0 {
		return
	}

	// Delete existing tags for this file first
	database.DB.Where("file_id = ?", fileID).Delete(&models.FileTag{})
	...
	for _, tagName := range tagNames {
		...
		database.DB.Create(&models.FileTag{
			FileID:  fileID,
			TagName: trimmed,
		})
	}
